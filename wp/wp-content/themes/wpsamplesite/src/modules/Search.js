...
class Search {
    ...
    getSearchURL(category) {
        return `${wpSampleSiteData.root_url}/wp-json/wp/v2/${category}?search=${this.searchField.val()}`;
    }

    getResults() {
        $.when(
            $.getJSON(this.getSearchURL('posts')),
            $.getJSON(this.getSearchURL('pages')),
        ).then((posts, pages) => {
